...
option_list <- list(
  make_option(c("-p", "--project_dir"), type="character", default="/Shared/vosslabhpc/symposia/cpsy-25/temp2",
              help="Project directory [default= %default]", metavar="character"),
  make_option(c("-d", "--project_deriv_dir"), type="character", default="/Shared/vosslabhp/symposia/cpsy-25/pacrad_ggiroutput",
              help="Project derivative directory [default= %default]", metavar="character"),
  make_option(c("-f", "--files"), type="character", default="\\\\itf-rs-store24.hpc.uiowa.edu\\vosslabhpc\\symposia\\cpsy-25\\temp2\\157 (2015-08-22)RAW.csv",
...
gather_subject_directories <- function(project_dir) {
  ...
  cat("Directories found:", directories, "\n")  # Debugging line
  subdirs <- directories[grepl("[0-9]", directories)]
  cat("Subject directories:", subdirs, "\n")  # Debugging line
  return(subdirs)
}

# Function to create output directories
create_output_directories <- function(project_dir, project_deriv_dir) {
  output_dir <- normalizePath(project_deriv_dir, mustWork = FALSE)
...
process_file <- function(r, project_dir, project_deriv_dir, verbose=FALSE) {
  ...
  SubjectGGIRDeriv <- function(x) {
    output <- file.path(project_deriv_dir, basename(x))
    return(output)
  }

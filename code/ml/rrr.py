...
class RRR:
    ...
    def build_model(self, X, Y):
        ...
        with pm.Model() as model:
            ...
            intercept = pm.Normal("intercept", mu=0, sigma=1, shape=(q,))
            mu_Y = pm.Deterministic("mu_Y", pt.dot(latent, B.T) + intercept)
            sigma = pm.HalfNormal("sigma", sigma=0.2, shape=(1, q))
...
def main():
    ...
    # Train the model
    trace = model.train(X, Y, draws=1000, tune=3000, target_accept=0.99)

    # Print the numerical summary
    print(az.summary(trace, var_names=["A", "B", "sigma"], round_to=4, stat_focus="mean").to_string())
